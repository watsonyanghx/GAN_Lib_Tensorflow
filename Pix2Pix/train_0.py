...
parser.add_argument('--conv_type', type=str, default='separable_conv2D', help='')
...
def create_model(inputs, targets):
    ...
    outputs = model.get_generator(inputs, out_channels, ngf=64, net_type='UNet', reuse=False)

    with tf.name_scope("real_discriminator"):
        # 2x [batch, height, width, channels] => [batch, 30, 30, 1]
        predict_real = model.get_discriminator(inputs, targets, ndf=64, spectral_normed=True,
                                               update_collection=None, net_type='UNet', reuse=False)

    with tf.name_scope("fake_discriminator"):
        # 2x [batch, height, width, channels] => [batch, 30, 30, 1]
        predict_fake = model.get_discriminator(inputs, outputs, ndf=64, spectral_normed=True,
                                               update_collection=None, net_type='UNet', reuse=True)
        ...
        if args.loss_type == 'WGAN-GP':
            ...
            with tf.variable_scope("discriminator", reuse=True):
                gradients = tf.gradients(
                    model.get_discriminator(inputs, interpolates, ndf=64, spectral_normed=True,
                                            update_collection=None, net_type='UNet', reuse=True), [interpolates])[0]
    ...
    with tf.name_scope("discriminator_train"):
        ...
        discrim_optim = tf.train.AdamOptimizer(args.lr, beta1=args.beta1, beta2=args.beta2)
    ...
    with tf.name_scope("generator_train"):
        with tf.control_dependencies([discrim_train]):
            ...
            gen_optim = tf.train.AdamOptimizer(args.lr, beta1=args.beta1, beta2=args.beta2)
...
def train():
    ...
    if args.mode == "test" or args.mode == "export":
        ...
        # with open(os.path.join(args.checkpoint_dir, "options.json")) as f:
