...
#         inputs = tf.transpose(inputs, [0, 3, 1, 2], name="NHWC_to_NCHW")
#
...
#             offset = tf.get_variable(name='offset', shape=[inputs.get_shape()[1], ], dtype=tf.float32,
#                                      initializer=tf.constant_initializer(0.))
#             scale = tf.get_variable(name='scale', shape=[inputs.get_shape()[1], ], dtype=tf.float32,
...
#                 mean = ((1. / batch_size) * mean) + (((batch_size - 1.) / batch_size) * moving_mean)[None, :, None,
#                                                     None]
#                 var = ((1. / batch_size) * var) + (((batch_size - 1.) / batch_size) * moving_variance)[None, :, None,
#                                                   None]
...
#                 return tf.transpose(outputs[:, :, :, 0], [0, 2, 3, 1], name='NCHW_to_NHWC')
#             else:
#                 # return outputs
...
#             if 0 not in axes:
#                 print("WARNING ({}): didn't find 0 in axes, but not using separate BN params for "
#                       "each item in batch".format(name))
#                 shape[0] = 1
#             offset = tf.get_variable(name='offset', shape=shape, dtype=tf.float32,
#                                      initializer=tf.constant_initializer(0.))
#             scale = tf.get_variable(name='scale', shape=shape, dtype=tf.float32,
#                                     initializer=tf.constant_initializer(1.))
#             result = tf.nn.batch_normalization(inputs, mean, var, offset, scale, 1e-5)
#
#             # return result
#             return tf.transpose(result, [0, 2, 3, 1], name='NCHW_to_NHWC')
