...
class ACGAN(object):
    ...
    def get_generator(self, z_var, labels=None, training=True, reuse=False):
        ...
        with tf.variable_scope('g_net', reuse=reuse):
            ...
            # (N, 4, 4, 512)
            output = lib.ops.linear.Linear(z_var_, z_var_.shape.as_list()[-1], 4 * 4 * 128, 'G.Input')
            output = tf.reshape(output, [-1, 4, 4, 128])
            # print('G.Input.shape: {}'.format(output.shape.as_list()))

            output = ResidualBlock(output, output.shape.as_list()[-1], 128, 3, 'G.1',
                                   resample='up', labels=labels, activation_fn='relu')
            output = ResidualBlock(output, output.shape.as_list()[-1], 128, 3, 'G.2',
                                   resample='up', labels=labels, activation_fn='relu')
            output = ResidualBlock(output, output.shape.as_list()[-1], 128, 3, 'G.3',
                                   resample='up', labels=labels, activation_fn='relu')
